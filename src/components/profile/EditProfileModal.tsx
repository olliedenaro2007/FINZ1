...
export default function EditProfileModal() {
  ...
  async function save() {
    ...
    if (profileErr) { showToast('⚠ Save failed: ' + profileErr.message); setSaving(false); return }
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14, marginTop:4 }}>
          ...
          <div>
            <label className="btn btn-outline btn-sm" style={{ cursor:'pointer' }}>
              Upload Photo
              <input type="file" accept="image/*" style={{ display:'none' }} onChange={onPicChange} />
            </label>
      ...
      <div className="form-group">
        ...
        <div style={{ marginTop:4 }}>
          <label className="btn btn-outline btn-sm" style={{ cursor:'pointer' }}>
            ...
            <input type="file" accept="image/*" style={{ display:'none' }} onChange={onBgChange} />
          </label>
          {bgFileName && (
            <div style={{ fontSize:11, color:'var(--green)', marginTop:6 }}>✓ {bgFileName} selected</div>
          )}
          <div className="bg-presets" style={{ marginTop:10 }}>
            {BG_PRESETS.map(g => (
              <div key={g} className={`bg-preset${bgGradient === g ? ' sel' : ''}`}
                style={{ background: g }}
                onClick={() => { setBgGradient(g); setBgFile(null); setBgFileName(null) }} />
            ))}
          </div>
        </div>
      </div>
