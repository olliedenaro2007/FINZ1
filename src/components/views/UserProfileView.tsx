...
export default function UserProfileView() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      const [profRes, postsRes, fcntRes, fgcntRes, followRes] = await Promise.all([
        ...
        supabase.from('follows').select('id', { count: 'exact', head: true }).eq('follower_id', viewingUserId!),
        user
      ...
      setFollowingCount(fgcntRes.count ?? 0)
